...
class EasyInstall:
    ...
    def __call__(self, *args, **kwargs):
        ...
        # Successfull package
        cprint("The packages have been successfully installed.",
               'white', 'on_red', attrs=['bold'])
    ...
    def __add__(self):
        ...
        cprint("{}.service file created.".format(
            self.gunicorn_file), 'white', 'on_red', attrs=['bold'])
        ...
        subprocess.call("sudo systemctl start {0} & sudo systemctl enable {0}".format(
            self.gunicorn_file), shell=True)
        cprint("Gunicorn was registered and restarted.",
               'white', 'on_red', attrs=['bold'])
        ...
        with open("{}/client/file/nginx".format(BASE_DIR)) as nginx_files:
            ...
            file_nignx = open(
                "{}/package/{}".format(BASE_DIR, self.nginx_file), 'w')
            file_nignx.write(nginx_file)
            file_nignx.flush()
            file_nignx.close()

        cprint("{} file created.".format(self.nginx_file),
               'white', 'on_red', attrs=['bold'])
    ...
    def requirements(self):
        ...
        cprint("requirements.txt successfully loaded.!",
               'white', 'on_red', attrs=['bold'])
    ...
    def save(self):
        ...
        cprint("/home/{}.json file created...".format(self.project_name),
               'white', 'on_red', attrs=['bold'])
...
def nginx_restart():
    ...
    subprocess.call("sudo systemctl restart nginx", shell=True)
    cprint("Nginx has been successfully restarted.",
           'white', 'on_red', attrs=['bold'])
...
def gunicorn_restart(project_name):
    ...
    with open("/home/{}.json".format(project_name)) as gunicorn_file:
        ...
        subprocess.call("sudo systemctl restart {}".format(
            data['gunicorn_file']), shell=True)

    cprint("Gunicorn has been successfully restarted.",
           'white', 'on_red', attrs=['bold'])
...
def RunEasy():
    ...
    # guniorn file name
    while True:
        cprint("Write your gunicorn file name",
               'white', 'on_red', attrs=['bold'])
    ...
    while True:
        ...
        if nginx_file == "":
            ...
            cprint("Write your virtualenv file path",
                   'white', 'on_red', attrs=['bold'])
            cprint("Example : /home/DjangoEnv",
                   'white', 'on_red', attrs=['bold'])
            break
    ...
    # project file
    while True:
        cprint("Write your Project file path",
               'white', 'on_red', attrs=['bold'])
        ...
        if project_file == "":
            ...
            if(os.path.isdir(project_file)):
                ...
                cprint("No such file or directory",
                       'white', 'on_red', attrs=['bold'])
                continue
    project_name = str(project_file).split(
        '/')[len(str(project_file).split('/'))-1]

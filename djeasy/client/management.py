...
class EasyInstall:
    ...
    def __init__(
        self, project_name, server_name_or_ip, static_url, gunicorn_file, nginx_file, project_file, virtualenv_file
    ):
        """
    ...
    def __add__(self):
        ...
        subprocess.call(
            "sudo cp {}/package/{}.service /etc/systemd/system/".format(BASE_DIR, self.gunicorn_file), shell=True
        )
        ...
        with open("{}/client/file/nginx".format(BASE_DIR)) as nginx_files:
            nginx = nginx_files.read().format(
                self.server_name_or_ip, self.static_url, self.project_file, self.project_name
            )
        ...
        # File move operations
        subprocess.call(
            "sudo cp {}/package/{} /etc/nginx/sites-available/".format(BASE_DIR, self.nginx_file), shell=True
        )

        # Nginx recording and restart
        subprocess.call(
            "sudo ln -s /etc/nginx/sites-available/{} /etc/nginx/sites-enabled".format(self.nginx_file), shell=True
        )
    ...
    def requirements(self):
        ...
        subprocess.call(
            '{}/bin/pip3 install -r {}/requirements.txt'.format(self.virtualenv_file, self.project_file), shell=True
        )
...
def RunEasy():
    ...
    easy = EasyInstall(
        project_name, server_name_or_ip, static_url, gunicorn_file, nginx_file, project_file, virtualenv_file
    )
    easy.__call__()
    easy.__add__()
    easy.__copy__()

...
class EasyInstall:
    ...
    def __add__(self):
        ...
        with open("{}/client/file/gunicorn.service".format(BASE_DIR)) as gunicorn_files:
            gunicorn = gunicorn_files.read().format(
                self.project_name, self.project_file, self.virtualenv_file
            )
            file_gunicorn = open(
                "{}/package/{}.service".format(BASE_DIR, self.gunicorn_file), "w"
            )
        ...
        cprint(
            "{}.service file created.".format(self.gunicorn_file),
            "white",
            "on_red",
            attrs=["bold"],
        )
        ...
        subprocess.call(
            "sudo cp {}/package/{}.service /etc/systemd/system/".format(
                BASE_DIR, self.gunicorn_file
            ),
            shell=True,
        )
        ...
        subprocess.call(
            "sudo systemctl start {0} & sudo systemctl enable {0}".format(
                self.gunicorn_file
            ),
            shell=True,
        )
        cprint(
            "Gunicorn was registered and restarted.", "white", "on_red", attrs=["bold"]
        )

        # nginx file save and move
        with open("{}/client/file/nginx".format(BASE_DIR)) as nginx_files:
            nginx = nginx_files.read().format(
                self.server_name_or_ip,
                self.static_url,
                self.project_file,
                self.project_name,
            )
        ...
        cprint(
            "{} file created.".format(self.nginx_file),
            "white",
            "on_red",
            attrs=["bold"],
        )
        ...
        # File move operations
        subprocess.call(
            "sudo cp {}/package/{} /etc/nginx/sites-available/".format(
                BASE_DIR, self.nginx_file
            ),
            shell=True,
        )
        ...
        subprocess.call(
            "sudo ln -s /etc/nginx/sites-available/{} /etc/nginx/sites-enabled".format(
                self.nginx_file
            ),
            shell=True,
        )
    ...
    def requirements(self):
        ...
        subprocess.call(
            "{}/bin/pip3 install -r {}/requirements.txt".format(
                self.virtualenv_file, self.project_file
            ),
            shell=True,
        )

        subprocess.call(
            "{}/bin/pip3 install gunicorn".format(self.virtualenv_file), shell=True
        )
        cprint(
            "requirements.txt successfully loaded.!", "white", "on_red", attrs=["bold"]
        )
    ...
    def save(self):
        ...
        with open("{}/client/file/server.info".format(BASE_DIR)) as server_file:
            server_file = server_file.read().format(
                self.project_name,
                self.server_name_or_ip,
                self.static_url,
                self.gunicorn_file,
        ...
        cprint(
            "/home/{}.json file created...".format(self.project_name),
            "white",
            "on_red",
            attrs=["bold"],
        )
        ...
        subprocess.call(
            "sudo chmod -R 777 /home/{}.json".format(self.project_name), shell=True
        )
...
def gunicorn_restart(project_name):
    ...
    with open("/home/{}.json".format(project_name)) as gunicorn_file:
        ...
        subprocess.call(
            "sudo systemctl restart {}".format(data["gunicorn_file"]), shell=True
        )

    cprint(
        "Gunicorn has been successfully restarted.", "white", "on_red", attrs=["bold"]
    )
...
def RunEasy():
    ...
    while True:
        cprint(
            "Please type in the server ip or domain address.",
            "white",
            "on_red",
            attrs=["bold"],
        )
        ...
        if server_name_or_ip == "":
            ...
            if domain(server_name_or_ip) or ipv4(server_name_or_ip):
                ...
                cprint(
                    "Please enter a valid address...", "white", "on_red", attrs=["bold"]
                )
                continue
    ...
    while True:
        cprint(
            "Write your STATIC_URL (Django Settings.py)",
            "white",
            "on_red",
            attrs=["bold"],
        )
